...
object ImageOptimizer {
    ...
    fun optimizeAndResize(inputBytes: ByteArray, format: String, resizeTo: Int = 128): ByteArray =
        ByteArrayInputStream(inputBytes).use { inputStream ->
            ...
            // Create temporary files to store the input and output images
            val tempInputFile = Files.createTempFile("inputImage", ".$format")
            ImageIO.write(inputImage, format, tempInputFile.toFile())

            val tempOutputFile = Files.createTempFile("outputImage", ".$format")

            // Create the ImageMagick command
            val cmd = ConvertCmd()
            // Prepare the operation
            val operation = IMOperation().apply {
                addImage(tempInputFile.toString()) // input
                resize(targetWidth, targetHeight)
                strip()
                addImage(tempOutputFile.toString()) // output
            }
            // Execute the operation
            try {
                cmd.run(operation)
            ...
            val outputBytes = Files.readAllBytes(tempOutputFile)
